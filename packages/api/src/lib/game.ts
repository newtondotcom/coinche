...
import type { IPlayer, IGameState } from "@coinche-reborn/api";

// Server will be injected via setServer method
let server: any = null;
...
export default class controller {
  ...
  public sendState() {
    ...
    if (!server) {
      logger.error(`Cannot send state: server not initialized for gameId: ${this.state.gameId}`);
      return;
    }
  ...
  public testState() {
    ...
    if (this.state.currentRound.tricks.length > 0 || this.state.currentRound.biddings.length > 0) {
      if (this.state.status !== "playing") {
        ...
        throw new Error(
          'Invalid game status: must be "playing" if there are ongoing rounds or biddings.',
        );
      }
    }

    if (this.state.gameId && this.state.status) {
