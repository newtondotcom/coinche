import controller from "../game";
import logger from "../logger";

export default async function addTrick(playerStartingId: string, gameId: string) {
  const controllerInstance = controller.getInstance(gameId);
  const currentRound = controllerInstance.getCurrentRound();
  const trickInit = {
    number: currentRound.tricks.length + 1,
    plays: [],
    playerStartingId: playerStartingId,
    team1Score: 0,
    team2Score: 0,
  };
  controllerInstance.state.currentRound.tricks.push(trickInit);
  logger.info("New trick created");
  controllerInstance.sendState();
}
