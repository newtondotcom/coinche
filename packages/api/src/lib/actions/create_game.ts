import { events } from "@coinche-reborn/db/schema/index";
...
export async function emitGameCreation(gameId: string) {
  ...
  const datas = await db
    .select()
    .from(events)
    .where(and(eq(events.type, "join"), eq(events.gameId, gameId)))
  ...
  if (datas?.length === 0) {
    ...
    const datas = await db
      .select()
      .from(events)
      .where(and(eq(events.type, "score"), eq(events.gameId, gameId)))
      .orderBy(desc(events.createdAt))
      .limit(1);
    if (datas?.length === 0) {
