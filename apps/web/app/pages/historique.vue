<template>
  <div
    class="flex flex-col justify-center items-center min-h-screen bg-gradient-to-b from-primary/5 to-background/80 py-8"
  >
    <div class="bg-white dark:bg-neutral-900 rounded-xl shadow-xl p-8 w-full max-w-2xl">
      ...
      <!-- Header Row -->
      <div
        class="flex flex-row space-x-6 justify-center items-center border-b-2 border-primary/30 pb-2 mb-2 bg-primary/10 rounded-t-lg"
      >
      ...
      <template v-if="isLoading">
        <div
          v-for="i in 6"
          :key="i"
          class="flex flex-row space-x-6 justify-center items-center border px-4 py-3 rounded-lg my-2 w-full animate-pulse bg-primary/5"
        >
      ...
      <template v-else>
        ...
        >
          <div class="text-xl font-semibold w-[120px] text-center truncate">
            {{ row.id || row.gameId }}
          </div>
          ...
          <div class="text-lg w-[100px] text-center">
            ...
            {{ row.team2_score ?? row.team2Score ?? 0 }}
          </div>
        </div>
        <div v-if="historique.length === 0" class="text-center text-neutral-500 mt-8">
