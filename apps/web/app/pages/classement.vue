    ...
    <div class="bg-white dark:bg-neutral-900 rounded-xl shadow-xl p-8 w-full max-w-xl">
      ...
      <template v-else>
        ...
        >
          ...
          <div class="text-xl font-semibold w-[80px] text-center">
            {{ row.points ?? row.totalPoints ?? row.score ?? 0 }}
          </div>
        </div>
      </template>
    </div>
...
const { data: classement, isLoading } = useQuery($orpc.leaderboard.queryOptions());
</script>
